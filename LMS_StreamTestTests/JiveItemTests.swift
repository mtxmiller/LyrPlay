...
final class JiveItemTests: XCTestCase {
    ...
    // MARK: - Mix actions (CarPlay "Start Mix", GH #85)

    /// Fixture mirrors the live `trackinfo items track_id:53421 menu:1` shape
    /// verified against 192.168.1.8 (Bliss installed) on 2026-05-30: a
    /// "Create bliss mix" item (go = blissmixer/mix), two "Similar tracks"
    /// items (go = blissmixer/list), and an ordinary "Play" item.
    private func blissTrackInfoResult() -> [String: Any] {
        [
            "count": 4,
            "item_loop": [
                ["text": "Play",
                 "actions": ["go": ["cmd": ["playlistcontrol"],
                                    "params": ["cmd": "load", "track_id": 53421]]]],
                ["text": "Create bliss mix", "type": "redirect",
                 "actions": ["go": ["cmd": ["blissmixer", "mix"],
                                    "params": ["useContextMenu": 1, "track_id": 53421, "menu": 1]]]],
                ["text": "Similar tracks", "type": "redirect",
                 "actions": ["go": ["cmd": ["blissmixer", "list"],
                                    "params": ["byArtist": 0, "track_id": 53421, "menu": 1, "useContextMenu": 1]]]],
                ["text": "Similar tracks by artist", "type": "redirect",
                 "actions": ["go": ["cmd": ["blissmixer", "list"],
                                    "params": ["byArtist": 1, "track_id": 53421, "menu": 1, "useContextMenu": 1]]]],
            ],
        ]
    }

    func testMixActionsFindsBlissMixAndSeedsByTrackID() {
        let actions = JiveItem.mixActions(fromTrackInfoResult: blissTrackInfoResult())
        // Only the `mix` verb qualifies — not the two `list` items, not Play.
        XCTAssertEqual(actions.count, 1)
        XCTAssertEqual(actions[0].cmd, ["blissmixer", "mix"])
        // The fired CLI args carry the seed track_id and the server's own params.
        XCTAssertTrue(actions[0].cliArgs.contains("track_id:53421"),
                      "fired args must seed the mix by track_id: \(actions[0].cliArgs)")
        XCTAssertTrue(actions[0].cliArgs.contains("useContextMenu:1"))
    }

    func testMixActionsExcludesSimilarTracksListVerb() {
        let actions = JiveItem.mixActions(fromTrackInfoResult: blissTrackInfoResult())
        // The `list` verb browses similar tracks; "Start Mix" wants the
        // build-and-play `mix` verb only.
        XCTAssertFalse(actions.contains { $0.cmd == ["blissmixer", "list"] })
    }

    func testMixActionsEmptyWhenNoMixerPlugin() {
        // A server without Bliss/MusicIP/MusicSimilarity (or a radio item):
        // trackinfo has no mixer action, so the button must stay hidden.
        let noMixer: [String: Any] = [
            "item_loop": [
                ["text": "Play", "actions": ["go": ["cmd": ["playlistcontrol"], "params": ["cmd": "load"]]]],
                ["text": "Album: SABLE", "actions": ["go": ["cmd": ["browselibrary", "items"], "params": ["album_id": 3648]]]],
            ],
        ]
        XCTAssertTrue(JiveItem.mixActions(fromTrackInfoResult: noMixer).isEmpty)
    }

    func testMixActionsRecognizesOtherMixerApps() {
        // The recognition set is provider-agnostic across MIXER_APPS.
        let musicip: [String: Any] = [
            "item_loop": [
                ["text": "Create MusicIP mix",
                 "actions": ["go": ["cmd": ["musicip", "mix"], "params": ["track_id": 42]]]],
            ],
        ]
        let actions = JiveItem.mixActions(fromTrackInfoResult: musicip)
        XCTAssertEqual(actions.count, 1)
        XCTAssertEqual(actions[0].cmd, ["musicip", "mix"])
    }

    func testMixActionsHandlesEmptyResult() {
        XCTAssertTrue(JiveItem.mixActions(fromTrackInfoResult: [:]).isEmpty)
    }
}
